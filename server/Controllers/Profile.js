    ...
    try {
        ...
        if (!contactNuber || !gender || !_id) {
            return res.status(401).json({
                sucess: false,
                message: "Unable to get all required feilds in update Profile controler"
            })
        }
        // check for profile ID
        const userDetails = await User.findById(_id);
        const ProfileId = userDetails.additionalDetaisl;
...
exports.getUserDetailsData(async (req, res) => {
    try {
        ...
        const userDetails = await User.findById(id).populate("additonalDetials").exec();
        return res.status(201).json({
            sucess: true,
